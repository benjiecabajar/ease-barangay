...
const RequestCertificationModal = ({ isOpen, onClose, onSubmit, submissionStatus }) => {
  ...
  const [goodMoralFiles, setGoodMoralFiles] = useState({ govIdFront: null, govIdBack: null, proofOfResidency: null, communityTaxCert: null });

  const [idPreviews, setIdPreviews] = useState({ front: null, back: null });
  const [residencyPreviews, setResidencyPreviews] = useState({ govIdFront: null, govIdBack: null, utilityBill: null });
  const [indigencyPreviews, setIndigencyPreviews] = useState({ front: null, back: null });
  const [goodMoralPreviews, setGoodMoralPreviews] = useState({ govIdFront: null, govIdBack: null, proofOfResidency: null, communityTaxCert: null });
    ...
    if (file) {
      ...
      switch (formType) {
        case 'clearance':
          ...
          break;
        case 'residency':
          if (residencyPreviews[fileKey]) URL.revokeObjectURL(residencyPreviews[fileKey]);
          setResidencyFiles(prev => ({ ...prev, [fileKey]: file }));
          setResidencyPreviews(prev => ({ ...prev, [fileKey]: newPreviewUrl }));
          break;
        case 'indigency':
          if (indigencyPreviews[fileKey]) URL.revokeObjectURL(indigencyPreviews[fileKey]);
          setIndigencyFiles(prev => ({ ...prev, [fileKey]: file }));
          setIndigencyPreviews(prev => ({ ...prev, [fileKey]: newPreviewUrl }));
          break;
        case 'goodMoral':
          if (goodMoralPreviews[fileKey]) URL.revokeObjectURL(goodMoralPreviews[fileKey]);
          setGoodMoralFiles(prev => ({ ...prev, [fileKey]: file }));
          setGoodMoralPreviews(prev => ({ ...prev, [fileKey]: newPreviewUrl }));
          break;
        default:
          break;
      }
  ...
  const takePicture = () => {
    if (!videoRef.current || !cameraFor?.fileKey || !cameraFor?.formType) return;
    ...
    canvas.toBlob((blob) => {
      if (blob) {
        const capturedFile = new File([blob], `${formType}-${fileKey}-${Date.now()}.png`, { type: 'image/png' });
  ...
  const handleRemoveImage = (fileKey, formType) => {
    const setters = {
      clearance: { files: setIdImages, previews: setIdPreviews, currentPreviews: idPreviews },
      residency: { files: setResidencyFiles, previews: setResidencyPreviews, currentPreviews: residencyPreviews },
      indigency: { files: setIndigencyFiles, previews: setIndigencyPreviews, currentPreviews: indigencyPreviews },
      goodMoral: { files: setGoodMoralFiles, previews: setGoodMoralPreviews, currentPreviews: goodMoralPreviews },
    };
  ...
  const handleSubmit = (e) => {
    ...
    if (certType === 'Barangay Clearance') {
      submission.frontIdFile = idImages.front;
      submission.backIdFile = idImages.back;
    } else if (certType === 'Certificate of Residency') {
      submission.residencyFiles = residencyFiles;
    } else if (certType === 'Certificate of Indigency') {
      submission.indigencyFiles = indigencyFiles;
    } else if (certType === 'Certificate of Good Moral Character') {
      submission.goodMoralFiles = goodMoralFiles;
    }
  ...
  const isSubmitDisabled = () => {
    ...
    if (certType === 'Barangay Clearance') {
      ...
      if (!firstName.trim() || !lastName.trim() || !idImages.front || !idImages.back) {
        return true;
      }
    }

    if (certType === 'Certificate of Residency') {
      if (!personalInfo.firstName.trim() || !personalInfo.lastName.trim() || !residencyFiles.govIdFront || !residencyFiles.govIdBack || !residencyFiles.utilityBill) {
        return true;
      }
    }

    if (certType === 'Certificate of Good Moral Character') {
      const { govIdFront, govIdBack, proofOfResidency, communityTaxCert } = goodMoralFiles;
      const { firstName, lastName } = personalInfo;
      if (!firstName.trim() || !lastName.trim() || !govIdFront || !govIdBack || !proofOfResidency || !communityTaxCert) {
        return true;
      }
    }

    if (certType === 'Certificate of Indigency') {
      if (!personalInfo.firstName.trim() || !personalInfo.lastName.trim() || !indigencyFiles.front || !indigencyFiles.back) {
        return true;
      }
    }
        ...
        <form className="cert-form" onSubmit={handleSubmit}>
          ...
          {certType === 'Barangay Clearance' && (
            <div className="clearance-requirements">
              ...
              {renderNameInputs()}
              <h4 className="requirements-header" style={{ marginTop: '20px' }}>Valid ID (Front and Back)</h4>
                ...
                <div className="id-upload-container">
                  ...
                  {idPreviews.front ? (
                    <div className="id-preview">
                      ...
                      <button type="button" className="remove-id-btn" onClick={() => handleRemoveImage('front', 'clearance')}>
                        <FaTimes size={10} />
                      </button>
                    </div>
                  ) : (
                    <div className="evidence-buttons">
                      <label htmlFor="front-id-upload" className="evidence-btn">
                        ...
                        <input type="file" id="front-id-upload" accept="image/*" onChange={(e) => handleFileChange(e, 'front', 'clearance')} style={{ display: 'none' }} />
                      </label>
                ...
                <div className="id-upload-container">
                  ...
                  {idPreviews.back ? (
                    <div className="id-preview">
                      ...
                      <button type="button" className="remove-id-btn" onClick={() => handleRemoveImage('back', 'clearance')}>
                        <FaTimes size={10} />
                      </button>
                    </div>
                  ) : (
              ...
              <p className="id-upload-note">
                Please provide a clear picture of the front and back of one valid government-issued ID.
              </p>
            </div>
          )}
          ...
          {certType === 'Certificate of Residency' && (
            <div className="clearance-requirements">
              ...
              {renderNameInputs()}
              <h4 className="requirements-header" style={{ marginTop: '20px' }}>Requirements</h4>
              <div className="id-upload-section">
                <div className="id-upload-container">
                  <label>Valid ID (Front)</label>
                  {residencyPreviews.govIdFront ? (
                    <div className="id-preview">
                      <img src={residencyPreviews.govIdFront} alt="Government ID Front Preview" />
                      <button type="button" className="remove-id-btn" onClick={() => handleRemoveImage('govIdFront', 'residency')}>
                        <FaTimes size={10} />
                      </button>
                    </div>
                  ) : (
                ...
                <div className="id-upload-container">
                  ...
                  {residencyPreviews.govIdBack ? (
                    <div className="id-preview">
                      <img src={residencyPreviews.govIdBack} alt="Government ID Back Preview" />
                      <button type="button" className="remove-id-btn" onClick={() => handleRemoveImage('govIdBack', 'residency')}>
                        <FaTimes size={10} />
                      </button>
                    </div>
                  ) : (
                ...
                <div className="id-upload-container">
                  ...
                  {residencyPreviews.utilityBill ? (
                    <div className="id-preview">
                      <img src={residencyPreviews.utilityBill} alt="Utility Bill Preview" />
                      <button type="button" className="remove-id-btn" onClick={() => handleRemoveImage('utilityBill', 'residency')}>
                        <FaTimes size={10} />
                      </button>
                    </div>
                  ) : (
          ...
          {certType === 'Certificate of Indigency' && (
            <div className="clearance-requirements">
              <h4 className="requirements-header">Indigency Certificate Requirements</h4>
              {renderNameInputs()}
              <div className="id-upload-section">
                <div className="id-upload-container">
                  ...
                  {indigencyPreviews.front ? (
                    <div className="id-preview">
                      <img src={indigencyPreviews.front} alt="Government ID Front Preview" />
                      <button type="button" className="remove-id-btn" onClick={() => handleRemoveImage('front', 'indigency')}>
                        <FaTimes size={10} />
                      </button>
                    </div>
                  ) : (
                ...
                <div className="id-upload-container">
                  ...
                  {indigencyPreviews.back ? (
                    <div className="id-preview">
                      <img src={indigencyPreviews.back} alt="Government ID Back Preview" />
                      <button type="button" className="remove-id-btn" onClick={() => handleRemoveImage('back', 'indigency')}>
                        <FaTimes size={10} />
                      </button>
                    </div>
                  ) : (
          ...
          {certType === 'Certificate of Good Moral Character' && (
            <div className="clearance-requirements">
              <h4 className="requirements-header">Good Moral Certificate Requirements</h4>
              {renderNameInputs()}
              <div className="id-upload-section">
                {/* Valid ID */}
                <div className="id-upload-container">
                  ...
                  {goodMoralPreviews.govIdFront ? (
                    <div className="id-preview">
                      <img src={goodMoralPreviews.govIdFront} alt="Government ID Front Preview" />
                      <button type="button" className="remove-id-btn" onClick={() => handleRemoveImage('govIdFront', 'goodMoral')}>
                        <FaTimes size={10} />
                      </button>
                    </div>
                  ) : (
                ...
                <div className="id-upload-container">
                  ...
                  {goodMoralPreviews.govIdBack ? (
                    <div className="id-preview">
                      <img src={goodMoralPreviews.govIdBack} alt="Government ID Back Preview" />
                      <button type="button" className="remove-id-btn" onClick={() => handleRemoveImage('govIdBack', 'goodMoral')}>
                        <FaTimes size={10} />
                      </button>
                    </div>
                  ) : (
              ...
              <div className="id-upload-section" style={{ gridTemplateColumns: '1fr', marginTop: '12px' }}>
                {/* Proof of Residency */}
                <div className="id-upload-container">
                  <label>Proof of Residency</label>
                  {goodMoralPreviews.proofOfResidency ? (
                    <div className="id-preview">
                      <img src={goodMoralPreviews.proofOfResidency} alt="Proof of Residency Preview" />
                      <button type="button" className="remove-id-btn" onClick={() => handleRemoveImage('proofOfResidency', 'goodMoral')} >
                        <FaTimes size={10} />
                      </button>
                    </div>
                  ) : (
                ...
                <div className="id-upload-container">
                  ...
                  {goodMoralPreviews.communityTaxCert ? (
                    <div className="id-preview">
                      <img src={goodMoralPreviews.communityTaxCert} alt="Community Tax Certificate Preview" />
                      <button type="button" className="remove-id-btn" onClick={() => handleRemoveImage('communityTaxCert', 'goodMoral')} >
                        <FaTimes size={10} />
                      </button>
                    </div>
                  ) : (
          ...
          <div className="submit-section">
            <button type="submit" className="submit-cert-btn" disabled={isSubmitDisabled()}>
              Submit Request
            </button>
          </div>
        </form>
